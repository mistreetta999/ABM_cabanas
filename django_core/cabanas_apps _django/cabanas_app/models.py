...
class Alquiler(models.Model):
    """Modelo que representa un alquiler."""
...
class Registro(models.Model):
    """Modelo para registrar acciones en el sistema."""
    ACCIONES = [
    ...
    def __str__(self) -> str:
        return f"{self.accion} - {self.fecha.strftime('%Y-%m-%d %H:%M:%S') if self.fecha else 'N/A'}" if isinstance(self.fecha, type(None)) or hasattr(self.fecha, 'strftime') else f"{self.accion} - {self.fecha}"
