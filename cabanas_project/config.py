...
class AppConfig:
    ...
    def default_auto_field(self):
        """Return the default auto field for this app."""
    ...
    def create(cls, entry):
        ...
        if app_name is None:
            if app_config_class is None:
                raise ImproperlyConfigured(f"'{entry}' must be a valid AppConfig subclass.")
            try:
                app_name = getattr(app_config_class, 'name', entry)
            except AttributeError as exc:
                raise ImproperlyConfigured(f"'{entry}' must supply a name attribute.") from exc
    ...
    def get_model(self, model_name, require_ready=True):
        ...
        """
        if require_ready and self.apps:
            self.apps.check_models_ready()
        elif self.apps:
            self.apps.check_apps_ready()
        if self.models is None:
            raise LookupError(
                f"App '{self.label}' models are not loaded."
            )
        try:
    ...
    def get_models(self, include_auto_created=False, include_swapped=False):
        ...
        if self.models is None:
            return iter([])
        for model in self.models.values():  # type: ignore
            if model.get_meta().auto_created and not include_auto_created:  
                continue
            if model.get_meta().swapped and not include_swapped: 
                continue
            yield model

    def import_models(self):
        """
        Import models for this app.
